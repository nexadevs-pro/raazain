...
const ProductCardOrdinary = ({
  product,
  updateSignedInUser,
}: ProductCardProps) => {
  const [selectedColor, setSelectedColor] = useState<string>(product.color1);
  ...
  return (
    <section>
      {product.categories.map((category) => category.title === "The Ordinary") ? (
...
export default ProductCardOrdinary;
